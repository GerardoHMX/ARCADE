class Tetris {
    ...
    draw() {
        ...
        // Dibujar línea límite de altura
        this.drawHeightLimit();

        // Dibujar tablero
        for (let row = 0; row < this.rows; row++) {
            for (let col = 0; col < this.cols; col++) {
                if (this.board[row][col]) {
                    this.drawCell(col, row, this.board[row][col].color);
                }
            }
        }

        // Dibujar pieza actual y pieza fantasma
        this.drawCurrentPiece();
        this.drawGhostPiece();
    }

    drawHeightLimit() {
        // Dibujar línea límite como parte del canvas
        this.ctx.save();
        ...
        this.ctx.beginPath();
        this.ctx.moveTo(0, this.heightLimit * this.cellSize);
        this.ctx.lineTo(this.gameCanvas.width, this.heightLimit * this.cellSize);
        this.ctx.stroke();
        this.ctx.restore();
    }
    ...
    drawCell(context, x, y, color) {
        ...
        context.shadowBlur = this.level > 5 ? 5 : 10;
    ...
    drawGhostPiece() {
        ...
        while (this.isValidMove(this.currentPiece.shape, this.currentPiece.x, ghostY + 1)) {
            ghostY++;
        }
    ...
    drawHoldPiece() {
        ...
        if (this.holdPiece) {
            const offsetX = (4 - this.holdPiece.shape[0].length) * this.cellSize / 2;
    ...
    drawNextPiece() {
        ...
        if (this.nextPiece) {
            const offsetX = (4 - this.nextPiece.shape[0].length) * this.cellSize / 2;
